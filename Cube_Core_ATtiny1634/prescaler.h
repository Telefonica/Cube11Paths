 ...
 * 
 * 2017 jorge.rivera@11paths.com add ATtiny1634 compatibility 
 * 
 * 
*/
...
inline void setClockPrescaler(uint8_t clockPrescaler) {
  if (clockPrescaler <= CLOCK_PRESCALER_256) {
    ...
    // Enable change.
#if defined (__AVR_ATtiny1634__)  // jorge.rivera@11paths.com add ATtiny1634 compatibility 
    CCP = 0xD8; // write CCP value to allow CLKPR writes http://www.avrfreaks.net/forum/attiny-1634-clock-frequency
#else
...
inline uint8_t getClockPrescaler() {
  ...
  return (CLKPR & (_BV(CLKPS0) | _BV(CLKPS1) | _BV(CLKPS2) | _BV(CLKPS3)));
}

inline uint16_t getClockDivisionFactor() {
  return ((uint16_t)(1 <<  getClockPrescaler()));
}
...
inline void trueDelay(unsigned long ms)
{
  ...
  while (micros() - start < (unsigned long)((ms*1000) / getClockDivisionFactor()));
  
}

// Waits for #us# microseconds.
// NOTE: Please see comment above.
inline void trueDelayMicroseconds(unsigned long us)
{
  unsigned long start = micros();
  
  while (micros() - start < (unsigned long)(us / getClockDivisionFactor()) );
}
